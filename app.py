...
st.markdown("""
    ...
    * { font-family: 'Plus Jakarta Sans', sans-serif; }
    ...
    .hotspot-pill { padding: 4px 10px; background: #FEF2F2; color: #B91C1C; border-radius: 15px; font-size: 0.7rem; font-weight: 700; }
    </style>
...
# --- NORMALIZACIÓN PARA MAPA (Match exacto con GeoJSON) ---
def normalizar_para_mapa(muni):
    m = str(muni).upper().strip()
    mapping = {
        "BUGA": "GUADALAJARA DE BUGA",
        "CALI": "SANTIAGO DE CALI",
        "JAMUNDI": "JAMUNDÍ", "JAMUNDI": "JAMUNDÍ",
        "TULUA": "TULUÁ", "GUACARI": "GUACARÍ",
        "ANDALUCIA": "ANDALUCÍA", "LA UNION": "LA UNIÓN",
        "RIOFRIO": "RIOFRIO", "EL CERRITO": "EL CERRITO",
        "BOLIVAR": "BOLÍVAR"
    }
...
def check_auth():
    ...
    if not st.session_state.logged_in:
        st.markdown("<h2 style='text-align:center;'>Pulse Analytics | Acceso</h2>", unsafe_allow_html=True)
...
if check_auth():
    ...
    if opcion == "📝 Registro":
        ...
        if not df.empty:
            ...
            st.markdown(f"""
                <div class="pulse-hero">
                    <p style='margin:0; opacity:0.7;'>PROGRESO META GLOBAL</p>
            ...
            # Mapa Coroplético
            st.subheader("📍 Distribución Geográfica en el Valle")
            ...
            with col_map:
                try:
                    ...
                    st.error("No se pudo cargar el mapa. Verifica la conexión.")
        ...
        if not df.empty:
            ...
            if busqueda:
                res = df[df.astype(str).apply(lambda x: busqueda in x.values, axis=1)]

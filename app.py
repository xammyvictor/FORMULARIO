...
if check_auth():
    ...
    st.sidebar.markdown(
        f"<div style='background:#F1F5F9; padding:20px; border-radius:18px;'>"
        f"<p style='font-size:0.75rem; font-weight:700; color:#64748B;'>SESIÓN PULSE</p>"
        f"<p style='font-size:1.1rem; font-weight:800;'>{usuario.upper()}</p></div>",
        unsafe_allow_html=True
    )
    ...
    # ==============================
        ...
        if not df.empty:
                ...
                <div class="pulse-hero">
                    <div style="display:flex; justify-content:space-between;">
                        ...
                        <div style="text-align:right;">
                            ...
                            <p style="opacity:0.6;">Meta: {META_REGISTROS:,}</p>
                        </div>
                    </div>
            ...
            for col, (lab, val) in zip(
                [k1, k2, k3, k4],
                [("Hoy", v_hoy), ("8 días", v_8d), ("30 días", v_30d), ("Municipios", df['Ciudad'].nunique())]
            ):
                col.markdown(
                    f"<div class='pulse-kpi-card'><div class='kpi-label'>{lab}</div><div class='kpi-val'>{val:,}</div></div>",
                    unsafe_allow_html=True
                )
